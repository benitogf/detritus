...
**Resolve every applicable R-check explicitly and report the ledger.** R1–R11 are the checks that reason about the *relationship* between the diff and the rest of the repo, which is precisely what an author-side review is worst at seeing — so name each one's outcome (`pass` / `blocker` / `n/a`, per `core/review-rigor` → *Per-check outcome — closed enum*) rather than letting the narrative checklists stand in for them. An enumerated brief is read as the definition of the job: a check absent from the report was not silently fine, it was **not run**, and a self-review that returns clean without the ledger cannot distinguish those two. The ledger is what makes a *sampled* review visibly different from a complete one (`core/review-rigor` → *Review stamps*, and R2b's *enumerate the claim set* rule, which cites that same ledger) — the defect that survives to a posted reviewer is routinely one an applicable check would have named, in a pass that never asked it (e.g. a claim of universal coverage cleared without R3, or a green suite credited for wiring no test ever executed).
