...
## Post-merge: surface approval-body non-blocking observations (non-gating)

The step-1 exemption keeps a "LGTM"-style approval body from deadlocking the merge — but reviewers routinely approve *with* a real non-blocking observation in that body ("approving; **non-blocking**: this `fetch` ignores a 4xx and will loop"). That observation must not be **dropped** just because it rode in the approval body — the loop's contract is that **every** non-blocker is handled, not merely the ones posted as inline/issue comments. It cannot gate the merge — fixing it pre-merge would push a commit, stale the SHA-pinned approval, and force a needless re-approval round-trip (exactly the churn the exemption exists to avoid) — so, like the `/learn` incident companion above, it is surfaced **after** the merge lands, never before:

- **On the Merged terminal**, on **any** PR (footered or not), re-read the approving review's own body. If it carries non-blocking observations — anything beyond a bare approval: a "non-blocking" / "nit" / "follow-up" remark, or a concrete suggestion the reviewer explicitly chose not to block on — **surface them and offer `/gh-issue-create`** (`core/icebox`) to file a tracked follow-up. Routing is the user's post-delivery call — **offer it, don't auto-run it**; a bare "LGTM" body has nothing to surface and skips silently.
- Strictly downstream of the merge — same contract as the `/learn` companion: it cannot change the merge gate, keep the loop alive, or turn a merged-success into a hand-back. The merge is the deliverable; capturing the non-blocker is the companion.

Together with the accompanying-comment path (step 3, addressed *before* the merge), this closes the gap end-to-end: **inline / issue / `COMMENTED` non-blockers are fixed pre-merge, approval-body non-blockers are routed to a follow-up post-merge — none is ever silently merged past.**
...
- **Out-of-scope review ask** — a reviewer requests something `/gh-feedback-work` will not fold in (it classifies the item out-of-scope). Stop; report the ask and offer the follow-up icebox issue (`/gh-issue-create`; `core/icebox`) `/gh-feedback-work` surfaces. Do not stretch the PR to cover it.
...
- **Merged** — the merge gate fired and the PR merged (success). Two non-gating post-merge companions then ride alongside the terminal, never changing the outcome: on a 🍬-footered PR, a read routes the producing unit's `incidents[]` to `/learn` (*Post-merge: route the producing unit's incidents to `/learn`*); and on **any** PR, non-blocking observations in the approving review body are surfaced with an offer to file a follow-up (*Post-merge: surface approval-body non-blocking observations*).
