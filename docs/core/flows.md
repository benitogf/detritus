...
- **watch (optional terminal phase)** — after `deliver` opens a PR, a flow may hand it to `/babysit` (`flows/github/babysit`) to carry it to merge: it folds in review feedback as it arrives (via `/gh-feedback-work`) and merges the moment a **SHA-pinned human `APPROVED` review** covers the current HEAD. This does **not** violate "never merges — the human merge is the gate": the human approval *is* the gate, and `/babysit` only executes the mechanical merge it authorizes. Watch is opt-in per flow (see *PR-watch — the universal terminal phase* below), never a stage a flow may add on its own initiative to force a merge past review.
...
Don't conflate this **merge-watch** with the **review-watch**: `/caregiver` (`flows/github/caregiver`) watches a PR from the reviewer's seat — re-reviewing on each push/discussion by dispatching the one-shot `/gh-pr` per event, re-posting a verdict only on each material verdict-change (a tier flip or a changed blocker set), and **never merging** — a different job from `/babysit`'s single watch-to-merge. Both borrow the same event-watch primitive (`core/janitor-platforms`); only `/babysit` reaches `merged`.
...
- **Auto-chaining flows** — `/vibe` and `/gh-issue-work` **auto-chain** into `/babysit` per opened PR rather than merely offering. For `/vibe` the stakeholder is non-technical, so a "run `/babysit` next" note they can't action would be a silent drop (`core/completion`'s no-deferral rule). For `/gh-issue-work` (the interactive `/gh` flow) it is the deliberate default — safe because `/babysit` never merges without a fresh human approval on HEAD, never merges on its own authority, is haltable at any time, and takes an optional user-set silence budget (no fixed run cap) — with two carve-outs: an explicit "just open it, don't watch" opts out, and it does **not** auto-chain when entered as the PR-opener *for* an autonomous caller (`/smith`, `/forge`, sidecar), which own their own watch decision. In every case the human's review approval is the merge gate; autonomy ends at merge-on-approval, never at a self-approved merge.
