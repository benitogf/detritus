...
triggers:
  ...
  - cross-domain task
when: Internal. Loaded by a coder agent assigned the fullstack role — a single small, tightly-coupled task that owns both the backend and the frontend side of one slice.
related:
  - core/coder
  - roles/tech-lead
...
Composes `core/coder` and owns **both** the server and client side of a single slice. It is the right role when the tech-lead judged the work **small and tightly coupled** — the UI consumes an API shaped in the same change — so splitting it across two parallel agents would drift the API contract against its consumer and force a dirty merge (`roles/tech-lead` → *Partition*). It is **not** a new agent kind or a new control surface — it is the existing coder executor given a boundary that spans two domains.
...
- **Use only when small and coupled.** If the slice is large, it should have been split into separate backend/frontend tasks sequenced with `deps` — that is the tech-lead's call, not the coder's. A fullstack task that turns out large is a blocker to report for re-partition, not a license to sprawl.
