...
1. **Resolve the item** (per Pre-flight) and **claim it via `/todo-claim <id>`** so the parent and other sessions see it as in-flight. `/todo-claim` writes `status: in-progress`, stamps `claimedAt`, and (in a forked session) sets `forkSession`.
...
11. **On "next"** (or any equivalent advance signal): resolve the new top open item (per /todo-view's sort) and restart at step 1 on it. Continue until the open set is empty or the user stops.
...
- After Step 1 (`/todo-claim`), TodoWrite re-syncs showing the item as `in_progress`.
...
- Not a planner. The item's priority is already settled by `/todo-audit` / `/todo-idle` / `/todo-add`'s sub-agent inference; this skill processes the one item the user (or `--next`) targets.
...
- The protocol is OPT-IN per invocation. Don't auto-invoke `/todo-work` from `/todo-view` or `/todo-audit`; the user always chooses.
