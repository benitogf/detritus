...
- **Learning-loop footer.** The shipped KB PR carries the 📚 learning-loop footer — *minted* at the open-PR step per `core/kb-writeback` "Ship a lesson", with its form and preservation-on-rewrite owned by `flows/github/gh` convention #11. Do not restate the footer text here; it lives in those two canonical locations.
- **One KB change, one issue, one PR.** A `/grow` run ships a single KB delta — if the compliance check surfaced a second, unrelated failure mode, it gets its own issue and PR, not a bolt-on to this one. This is `flows/github/gh` convention #5 (one issue, one PR); grow inherits it rather than restating the rule.
