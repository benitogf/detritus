...
- **Learning-loop footer.** The shipped KB PR carries the 📚 learning-loop footer — *minted* at the open-PR step per `core/kb-writeback` "Ship a lesson", with its form and preservation-on-rewrite owned by `flows/github/gh` convention #12. Do not restate the footer text here; it lives in those two canonical locations.
