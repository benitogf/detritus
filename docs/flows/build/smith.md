...
`/smith`'s definition of done is `core/completion`'s, not a local variant. The build phase's **exit gate is the five done-conditions** there — every acceptance box `[x]` with evidence, the verification gate green, a clean `/gh-self-review` pass, no new deferral markers, and zero units left `blocked` — computed from the durable ledger (the scratchpad's *Acceptance criteria*), never from memory. The loop continues until all five hold; it does not exit, hand back as "done", or convert a remaining in-scope item into a deferral (`core/completion` → *Exit gate*).
