...
- **A control proves nothing if it shares the suspect state.** "It fails on the untouched base too, so it is pre-existing" holds only when the control could actually have come out differently. If both arms share the thing under suspicion — a stale generated artifact, a cached index, a build output, a populated database, an installed dependency, an environment variable — then an identical result is guaranteed by construction and proves only *not caused by my change*, never *pre-existing and not my problem*. Name what the two runs share before drawing the conclusion, and eliminate it (regenerate the artifact, clear the cache, use a clean checkout) so the control can dissent. The same rule governs any A/B check, sabotage, or bisect: **a comparison whose outcome could not have differed carries no information**.
