---
description: Platform-specific scheduling and watch adapters for the recurring-loop commands (/janitor, /smith) and the event-driven watches (/babysit's merge-gate watch, /caregiver's review-watch). Keeps Codex, Claude Code, GitHub Actions, Cursor, Windsurf, and generic scheduler/watch details out of the core workflows.
triggers:
  - janitor platform
  - janitor platforms
  - janitor scheduling
...
when: /janitor or /smith needs to create a recurring schedule on a specific platform, or /babysit (merge-gate watch) or /caregiver (review-watch) needs an event-watch adapter for the host — or a flow needs the nearest supported scheduler/watch behavior explained.
related:
  - core/loop
  - flows/build/janitor
  - flows/build/smith
...
- Every **poll-per-tick maintenance adapter** (the `/janitor` / `/smith` schedulers) must preserve target-scoped non-overlap: a wake checks existing branches, issues, PRs, or state before starting fresh audit work. The **event-watch adapter** (`/babysit`, `/caregiver`) does not participate in this — it watches a single named target and carries no scratchpad; non-overlap does not apply.
...
while true; do
  ...
  LC_ALL=C comm -13 <(echo "$prev") <(echo "$cur")   # SAME pinned collation as the sort -- comm compares by LC_COLLATE too
...
- **`comm` inputs must be GLOBALLY sorted, and `sort` and `comm` must share ONE pinned collation.** `comm` walks two streams assuming both are sorted **in its own collation**; a `cur` built by concatenating separately-sorted sub-fetches is **not** sorted. Sorting each sub-fetch is exactly the trap — it looks sorted and isn't. On unsorted input the walk desyncs and `comm` returns a wrong diff: lines present in **both** streams get re-emitted as new (`prev`=`state open / review A / inline 111`, `cur`=`state open / review B / inline 111` wrongly emits the unchanged `inline 111`), and once desynced the remaining classifications are no longer trustworthy in either direction. Phantom events are not the benign failure they look like: `Monitor` auto-stops a too-chatty watch, so a desynced diff can get the watch **killed**, and a dead watch is indistinguishable from a quiet PR (*silence is not success*, above). The only diagnostic (`comm: input is not in sorted order`) goes to **stderr**, which `Monitor` writes to the output file and never turns into an event — so nothing in the event stream reveals any of this. Fix: sort the **whole** assembly once, immediately before the compare and **after** the failure guard (sorting inside the `$( … )` makes the pipeline's exit status `sort`'s and silently defeats the inconclusive-poll guard above), and run **both** `sort` and `comm` under the same pinned collation. `comm` does **not** compare byte-wise — it honours `LC_COLLATE` exactly as `sort` does (`man comm`: *"comparisons honor the rules specified by `LC_COLLATE`"*) — so `LC_ALL=C sort` feeding an unpinned `comm` leaves the halves disagreeing in any non-C locale (`en_US.UTF-8` is the Linux default) and reproduces the very phantom event this rule exists to prevent. Pin the pair (`LC_ALL=C sort` … `LC_ALL=C comm`), or `export LC_ALL=C` once for the loop. Applies to **any** set-difference over a multi-source assembly, not just this watch.
