...
>
> Control returns to the user at exactly three points: **(a)** a terminal outcome — the PR merged, or was closed without merging; **(b)** a **pause & hand back** — an out-of-scope review ask or a non-clean merge the loop cannot resolve (*Stuck = pause & hand back*), or the **20-tick cap** (*Tick cap*); **(c)** an explicit user halt. **Everywhere else — including every ordinary idle tick — the tick MUST end with a live continuation path**: the automatic next wake armed (the default — a standing schedule or a firing self-wake), or, *only* when no automatic scheduler is available, an explicit **degraded-fallback** re-invoke hand-off (*Scheduling* → *Self-continuation*). Automatic is the default and normal path; on-demand is only an override or that degraded fallback — never what the loop silently settles into. Ending a tick with "waiting for review" and **neither** an automatic wake armed **nor** a degraded hand-off stated is the stall this contract exists to prevent.
...
`/babysit` is **automatic by default** — its normal operation is a timer that fires each tick, with no user action in between; the loop never *defaults* to "run one tick, wait for a nudge." On-demand exists only as an **override** and a **degraded fallback** (below), never as the default continuation. **Automatic arrangement** — use the first the host supports:
...
Under either automatic arrangement, every non-terminal tick MUST arm the next wake before yielding; a tick that ends with no next wake armed **and** no degraded-fallback hand-off (below) is the stall this contract forbids.

### On-demand: override and fallback (never the default)

- **Override — "tick now."** While an automatic watch is live, the user may invoke `/babysit <pr>` (or nudge) to run **one extra tick immediately**, between scheduled wakes — e.g. "I just approved, check now, don't wait for the interval." The standing schedule keeps running; the override is an additional user-initiated tick, **not** a replacement for the automatic continuation.
- **Fallback — degrade, don't refuse.** If the host can arm **neither** automatic arrangement, do **not** refuse to run — **degrade to on-demand**: run one tick per invocation and end with an explicit, clearly-labeled hand-off — *"degraded manual mode (no automatic scheduler on this host): re-invoke `/babysit <pr>` to run the next tick, or set up a durable cron for an unattended watch."* This is a legitimate continuation path (the user's re-invocation), but it is a **last resort**, flagged as degraded every time — never presented as normal operation. (Same honest-degradation spirit as `core/loop` → *Durability*: surface the mode mismatch, don't silently misbehave.)

The line to hold: **automatic is the default and the normal path; on-demand is an override the user reaches for, or a degraded fallback when nothing else fires — never what the loop silently settles into as its standard continuation.** Confirm which is live before the first status line; "I'll keep checking" with no automatic wake armed **and** no degraded-fallback hand-off stated is the stall. Stop arming/soliciting the next tick **only** at the hand-back points in *The loop must never stall*.
...
- **Automatic is the default; on-demand is override/fallback only, never the default.** The loop's normal continuation is an automatic next wake (standing schedule or firing self-wake). On-demand is available *only* as (a) a manual "tick now" **override** on top of a live schedule, or (b) a **degraded fallback** when the host can arm no automatic trigger — run one tick per invocation, flagged degraded every time, recommending a real scheduler. Never let the loop *default* to one-tick-per-invocation, and never *refuse to run* just because no scheduler exists — degrade instead (*Scheduling* → *Self-continuation*).
- **An idle tick continues the loop; it never silently stops.** Under the automatic default it re-arms the next wake; in degraded fallback it ends with the labeled re-invoke hand-off. "Waiting for review" is the normal state. Stopping without a terminal / hand-back / user-halt reason is this skill's #1 failure mode — a status line with no action and no continuation path is the tell.
