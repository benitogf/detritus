...
func main() {
	if len(os.Args) > 1 {
		...
		case "--init":
			...
			os.Exit(1)
		case "--update":
			// Usage: --update [<tag>] [--dry-run]. Any non-flag argument pins the
		...
		case "--help", "-h":
			...
			fmt.Println("  detritus --update [<tag>] [--dry-run]                 Self-update (verifies checksums; optional tag pins the release)")
