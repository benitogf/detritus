...
The shared scratchpad spine (current orientation, tick log, state block) is defined in `core/loop`. `/janitor` adds these sections above the tick log, in order:
...
The honest-regression rule (`core/loop` → *Shared Main Agent Rules*) applies regardless: any movement of a tracked metric in the wrong direction must surface in that tick's output.
...
The skip-streak guardrail (`core/loop` → *Skip-Streak Guardrail*) protects against silent drift even when no end criteria are set.
...
The first report follows `core/loop` → *Initial Run Reporting*. Concretely:
