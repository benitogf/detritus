...
Model and effort are set per role, never per command — the installed `~/.claude/agents/detritus-reviewer.md` definition runs the **configured review model** (default `claude-fable-5`) at the **configured effort** (default `high`), both set via `/settings`, for in-session spawns; candyland's settings default the reviewer roles to the same. `/settings reviewer model …` (and `… thinking …`) is the **durable** choice — it re-writes the agent definition for every future session — whereas the in-flow opus fallback below is a **transient**, single-session degrade. Independent review wants a strong model at high effort, not the builder's own model reviewing its own blind spots — which is why the defaults are `claude-fable-5`/`high`.
