...
After resolving target, topics or spec, and interval, load `meta/janitor-platforms` and choose the adapter for the detected or requested platform. The adapter decides how to schedule the loop and how to translate the user's human cadence into the platform's scheduler.
...
Keep all platform-specific APIs, cron syntax, CLI flags, and management URLs in `meta/janitor-platforms`. Both `/janitor` and `/smith` use the same adapter doc; the adapter doesn't need to know which loop is consuming it.
...
- Not a standalone workflow. Invoke `/janitor` or `/smith` (or whichever recurring-loop command applies), not `/loop-core`.
- Not a place to record command-specific safety boundaries, audit lenses, or end criteria. Those live in `meta/janitor` and `meta/smith` so each command's intent stays distinct.
- Not the platform adapter doc. Scheduler-specific behavior lives in `meta/janitor-platforms`.
