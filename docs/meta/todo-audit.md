...
triggers:
  ...
  - claim todo
  - I'm working on
when: User asks to re-prioritize ("what's next", "rank my todos"), wants a strategic between-tasks review (idle), asks whether todos can run in parallel (fork), or claims/releases an item inside a forked conversation. Also invoked by the main agent on detected pivots per meta/todo convention #8.
related:
  - meta/todo
  - meta/todo-import
  - meta/todo-work
...
**When to invoke**: explicit request (`/todo audit`, "re-prioritize", "what's next"), or pivot-detection by the main agent (`meta/todo` convention #8). The audit is not free (Sonnet tokens) — pivot-only, never every prompt:
