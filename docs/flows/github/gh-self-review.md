...
Spawn the review via the `Agent` tool. The sub-agent runs with **no prior context** — the prompt is everything it sees. Use `subagent_type: "detritus-reviewer"` — the role definition `detritus --setup` installs at `~/.claude/agents/detritus-reviewer.md`, which pins the reviewer's model and effort per role (`roles/reviewer` → *Model and effort*). If the definition is absent (setup not run), fall back to `subagent_type: "general-purpose"` so the review still runs — it then inherits the session model. If the reviewer spawn returns or dies with a model-limit banner, re-spawn it per `roles/reviewer` → *Model and effort* (In-session model-limit fallback).
