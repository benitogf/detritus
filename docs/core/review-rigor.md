...
- **Behavioral fixes for signal/event-driven or stateful code** ("fixes the stuck badge", "fixes the double-fire", "fixes the race") — a clean parse, a passing static/headless check, or a code review that reads the diff as logically sound is **not evidence** the fix works. The bug lived in a runtime sequence of events; the claim is proven only by executing that same sequence (a live debugger/editor drive, an on-device run, or an integration test that fires the real event order) and observing the actual outcome — not by re-reading the diff and confirming the logic looks right. Default position: blocker until the actual sequence has been driven and observed. Detection cue: a PR/issue claims a signal-ordering or multi-event bug is fixed, but the cited verification is parse/static-check/code-review only, with no run that fired the real event sequence. (e.g. a Godot fix gating badge/countdown display across `deck_change` → `start_bettimer` → `no_more_bets` passed `--check-only` and a diff review, then shipped semantically wrong — the multi-event suppression window was never actually driven end-to-end before merge; on-device testing caught it after the fact.)
