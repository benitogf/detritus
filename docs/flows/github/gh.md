...
| URL / ref resolves to an **open PR**, AND user text suggests reviewing it — phrases like "review pr", "review this pr", "code review", "hard review", "review pull request" | `gh-pr` — **note:** `/gh-pr` now defaults to a *watching* review (posts the verdict, then keeps re-reviewing until the PR is merged/closed). Dispatching here starts that watching review **unless** the user asked for a one-shot ("review once", "just review it", "don't watch", `--once`) — propagate that through as `gh-pr --once` (Phase 2). |
...
**Propagate the review-once signal.** `/gh-pr` (and `/gh-pr-safe`, which inherits it) defaults to a watching re-review. When the user's phrasing — in the `/gh` args or the invoking message — asks for a one-shot review ("review once", "just review it", "don't watch", "one and done", `--once`), carry that through to the sub-skill as `gh-pr --once` so it posts one verdict and stops. Absent such phrasing, the watch default stands. This is the same propagate-the-signal discipline as the authorization signal above.
