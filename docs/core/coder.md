...
- **TDD gate.** The task is defined by a failing test. The coder that owns the task writes that defining test **failing-first**, as the task's first step — its author is the task's own coder, never a sibling. "Done" means that test goes green and the canonical verification command passes — the same gate `core/build` enforces for a single build unit. A coder does not declare done on a red or unrun test.
  - **Sabotage-check a new assertion before trusting its green.** Failing-first proves the test fails when the feature is *absent*; it does **not** prove a new assertion can fail once the feature *exists*. A tautological or feature-blind assertion — comparing a value with itself, asserting an event *fired* but never *what it produced*, an error metric identically zero for any input — passes failing-first yet is vacuous. So a new assertion counts as coverage only once **sabotage-checked**: deliberately break the implementation it targets, confirm the assertion goes **red at the expected lines**, then restore and re-verify clean. A green assertion never shown able to fail against a broken implementation is **not verified** — never report it as such (green ≠ verified; `flows/principles/truthseeker` → *Prove Before Acting*).
    - **Confirm the sabotage actually applied before reading the result.** A mutation that silently fails to match — wrong indentation, `CRLF` vs `LF`, a line that moved — leaves the code intact, so the suite goes green and that green reads as "the assertion survived sabotage." That is the *false confidence the check exists to remove*. Prove the mutation is present before running (a grep count, a diff line count, a changed-byte check); an unverifiable mutation is **no evidence at all**, not a pass.
    - **A green sabotage is a finding, not a pass.** If the assertion stays green against a confirmed-broken implementation, the assertion cannot fail: fix the assertion and sabotage again. Never record the check as done on a green sabotage.
    - **One claim per sabotage, and break the *use*, not the presence.** A sabotage that changes two things at once goes red for whichever the suite notices first, so it cannot attribute that red to the assertion under test — the assertion can still be vacuous behind a neighbour that happens to be covered. Break exactly what this assertion claims to pin, leave every neighbour intact, and read *which* assertion failed rather than that something did. This is sharpest when the assertion can only reach a **proxy** for the behaviour — source text, a computed intermediate, a log line, a call count — because the proxy's *presence* and its *use* are separate facts, and only the second is the behaviour. The sabotage that counts there is the one that keeps the proxy and breaks the use (e.g. an assertion that a request-scoping value is computed stays green when only the request is changed to ignore it, so a compound sabotage that deletes both reports coverage the assertion does not have). If no sabotage can separate presence from use, the assertion is pinning the wrong thing: assert on the observable the code produces, and where the harness cannot observe it, say so rather than banking the proxy as coverage.
    - **Scope each assertion to the surface it guards.** A whole-file substring search is satisfied by unrelated text elsewhere in the file (the same identifier in a neighbouring function, a doc comment, the generator that emits it), so it survives deleting the very line it claims to pin. Match within the region under test, or assert on a form specific enough that only the guarded code satisfies it.
