...
- A **"review PR #N"** intent ("check PR #N against the requirements doc", "review #N") → `deliver: review`. The quest may end with **no PR at all** when nothing is actionable; any actionable finding becomes `feedback` work on the PR in question. A no-PR `review` terminal is valid `done`, not a no-op (`core/completion` → *Honest terminal state*, carve-out 3) — **but only once the PR was actually reviewed**: a review/feedback quest with a target PR must run a review of that PR before any "no findings" terminal, so it can never report `reviewed` on a PR it never read. If discovery surfaces nothing while the PR has not yet been reviewed, the review itself is the seeded work item, and it **launches even at L1** — for a targeted PR the review *is* the quest's job, the one narrow carve-out to L1's "surface only" rule (a review quest that launches nothing has done nothing).
