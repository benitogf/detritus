...
// sidecar detritus is driving this launch. They start at the default ports and
// ensureCandylandUp rewrites them to the discovered/launched endpoint, so every
// subsequent REST call and printed URL points at the real listener regardless of
// which ports were used. candyland owns the contract; detritus only consumes it.
var (
	candylandBaseURL      = fmt.Sprintf("http://127.0.0.1:%d", candylandDefaultAPIPort)
	candylandDashboardURL = fmt.Sprintf("http://localhost:%d", candylandDefaultSPAPort)
)
...
// endpoint file's advertised port first (verified via health), then the default
// ports. A stale endpoint file (dead port) fails verification and falls through.
// found is false when nothing verifies.
func resolveExistingCandyland(timeout time.Duration) (candylandResolution, bool) {
	...
	base, dash := candylandURLs(candylandDefaultAPIPort, candylandDefaultSPAPort)
	if id, err := candylandIdentityAt(base, timeout); err == nil {
		return candylandResolution{id: id, baseURL: base, dashURL: dash, apiPort: candylandDefaultAPIPort, spaPort: candylandDefaultSPAPort}, true
	}
