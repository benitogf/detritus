...
If, after updating and restarting, a `kb_get`/`kb_list` call still returns "not found" for a doc you know exists in this release (e.g. a newly added `flows/*` or `core/*` doc), the cached bootstrap binary is stale and wasn't cleared — e.g. the running binary predates automatic invalidation, or `DETRITUS_CACHE_DIR` points somewhere `--setup` didn't expect. Confirm with `"${DETRITUS_CACHE_DIR:-$HOME/.cache}/detritus-codex/detritus" --version`, then clear it manually and restart the client:
